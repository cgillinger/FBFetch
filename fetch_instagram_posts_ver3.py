...
# Version 2.0 - Instagram Post-nivå Analytics (Moderniserad)
# 
...
#
# MAJOR UPDATE v2.0: Använder moderna datumfilter för dramatisk prestandaförbättring
# - Ersätter ineffektiv paginering genom hela historiken
# - Använder since/until parametrar på /media endpoint (Graph API v11.0+)
# - Sekunder istället för minuter per månadskörning
# - Eliminerar alla ordningsproblem och saknade posts
#
# VIKTIGA NOTERINGAR:
...
# - Stödjer datumfiltrering direkt via API (v11.0+)
#
...
# ===================================================================================
# ===================================================================================
# HÄR BÖRJAR DEL 2 - Post-hämtning och Insights-integrering (MODERNISERAD v2.0)
# ===================================================================================
...
def get_instagram_posts_for_period(instagram_id, since_date, until_date, account_name=None):
    ...
    v2.1 KRITISKA FIXAR baserat på feedback:
    - Korrekt tidszonhantering (UTC → Sverige/Europa)
    - Eliminerat för tidig stopp-logik som orsakade saknade posts
    - Förbättrad server-side filtrering med fallback
    - Detaljerad diagnostik för att identifiera saknade posts
    
    TEKNISK FÖRKLARING:
    Använder Instagram Graph API:s since/until parametrar men med robust
    tidszonhantering och utan lokal stopp-logik som kan orsaka luckor.
    ...
    logger.info(f"Hämtar posts för {display_name} från {since_date} till {until_date} (robust datumfiltrering v2.1)")
    
    try:
        from datetime import datetime, timezone
        import pytz
        
        # TIDSZONHANTERING: Konvertera svenska datumgränser till UTC för API
        sweden_tz = pytz.timezone('Europe/Stockholm')
        
        # Startdatum: 00:00 svensk tid
        start_sweden = sweden_tz.localize(datetime.strptime(since_date, "%Y-%m-%d"))
        start_utc = start_sweden.astimezone(pytz.UTC)
        
        # Slutdatum: 23:59:59 svensk tid  
        end_sweden = sweden_tz.localize(datetime.strptime(until_date + " 23:59:59", "%Y-%m-%d %H:%M:%S"))
        end_utc = end_sweden.astimezone(pytz.UTC)
        
        # Formatera för API (ISO 8601)
        since_utc_str = start_utc.strftime("%Y-%m-%dT%H:%M:%S")
        until_utc_str = end_utc.strftime("%Y-%m-%dT%H:%M:%S")
        
        logger.debug(f"  Tidszonkonvertering:")
        logger.debug(f"    Sverige: {since_date} 00:00 → {until_date} 23:59")  
        logger.debug(f"    UTC: {since_utc_str} → {until_utc_str}")
        
        posts = []
        url = f"https://graph.facebook.com/{API_VERSION}/{instagram_id}/media"
        
        # FÖRBÄTTRAD SERVER-SIDE FILTRERING: Använd UTC-konverterade datum
        params = {
            "access_token": ACCESS_TOKEN,
            "since": since_utc_str,        # UTC-konverterat startdatum
            "until": until_utc_str,        # UTC-konverterat slutdatum
        ...
        total_posts_found = 0
        posts_in_period = 0
        posts_filtered_by_type = 0
        posts_outside_period = 0
        
        logger.debug(f"  Startar API-anrop med server-side filtrering...")
        
        # ROBUST PAGINERING: Ingen lokal stopp-logik som kan orsaka luckor
        while url:
            page_num += 1
            logger.debug(f"Hämtar sida {page_num} för {display_name}...")
            ...
            if data and "data" in data:
                ...
                logger.debug(f"  API returnerade {len(media_in_page)} posts på sida {page_num}")
                
                # Tom sida = naturligt slut
                if len(media_in_page) == 0:
                    logger.debug(f"  Nått naturligt slut - inga fler posts")
                    break
                
                # BEARBETA ALLA POSTS - ingen tidig stopp-logik
                for post in media_in_page:
                    post_timestamp = post.get("timestamp", "")
                    
                    if post_timestamp:
                        try:
                            # KORREKT TIDSZONHANTERING: Parseera UTC timestamp
                            post_utc = datetime.strptime(post_timestamp, "%Y-%m-%dT%H:%M:%S%z")
                            if post_utc.tzinfo is None:
                                post_utc = pytz.UTC.localize(post_utc)
                            
                            # Konvertera till svensk tid för jämförelse
                            post_sweden = post_utc.astimezone(sweden_tz)
                            post_date = post_sweden.strftime("%Y-%m-%d")
                            
                            logger.debug(f"    Post: {post_timestamp} UTC → {post_date} Sverige")
                            
                            # KONTROLLERA SVENSK DATUMPERIOD
                            if since_date <= post_date <= until_date:
                                ...
                                if media_product_type in ["FEED", "REELS"]:
                                    ...
                                    logger.debug(f"    ✓ Inkluderad: {post_date} {media_type}/{media_product_type}")
                                else:
                                    posts_filtered_by_type += 1
                                    logger.debug(f"    ✗ Filtrerad post-typ: {media_product_type}")
                            else:
                                posts_outside_period += 1
                                logger.debug(f"    ✗ Utanför period: {post_date}")
                                
                        except Exception as e:
                            logger.warning(f"    ⚠ Fel vid tidskonvertering för post {post.get('id', 'unknown')}: {e}")
                            continue
                
                # Fortsätt paginering till naturligt slut
                paging = data.get("paging", {})
                if "next" in paging:
                    url = paging["next"]
                    params = {}  # Parametrar finns i URL
                    logger.debug(f"    Fortsätter till nästa sida...")
                else:
                    url = None
                    logger.debug(f"    Nått slutet av tillgänglig data")
                        
            elif data and "error" in data:
                error_msg = data["error"].get("message", "Okänt fel")
                error_code = data["error"].get("code", "N/A")
                
                # Kontrollera server-side filtrering support
                if any(term in error_msg.lower() for term in ["since", "until", "parameter"]):
                    logger.warning(f"Server-side filtrering kanske inte stöds: {error_msg}")
                    logger.warning(f"Använder fallback utan datumfilter...")
                    # TODO: Implementera fallback utan since/until
                    
                logger.error(f"Error {error_code}: {error_msg}")
                break
            else:
                logger.warning(f"  Inget data returnerat för {display_name}")
                break
            
            # Säkerhetsbroms för oväntade fall
            if page_num > 100:
                logger.warning(f"Säkerhetsbroms: {page_num} sidor - avslutar för säkerhets skull")
                break
        
        # DETALJERAD DIAGNOSTIK för att identifiera problem
        logger.info(f"  ✓ Resultat för {display_name} ({since_date} → {until_date}):")
        logger.info(f"    • {total_posts_found} posts returnerade av API totalt")
        logger.info(f"    • {posts_in_period} posts inom svensk tidsperiod (INKLUDERADE)")
        logger.info(f"    • {posts_filtered_by_type} posts filtrerade bort (Stories etc)")
        logger.info(f"    • {posts_outside_period} posts utanför tidsperiod")
        logger.info(f"    • {page_num} sidor paginerade")
        
        # VARNING om få posts för aktiv månad
        if posts_in_period < 5 and page_num == 1:
            logger.warning(f"    ⚠ Förvånansvärt få posts ({posts_in_period}) - kontrollera server-side filtrering")
            
        return posts
            
    except ImportError:
        logger.error("pytz-biblioteket krävs för tidszonhantering. Installera med: pip install pytz")
        return []
    except Exception as e:
...
def process_posts_with_insights(posts, account_name=None, instagram_account_id=None):
    ...
    PRESTANDAFÖRBÄTTRING v2.0:
    Med modern datumfiltrering bearbetar denna funktion betydligt färre posts
    per körning, vilket resulterar i dramatiskt snabbare exekvering.
...
# ===================================================================================
# HÄR SLUTAR DEL 2 - Post-hämtning och Insights-integrering (MODERNISERAD v2.0)
# ==================================================================================='# ===================================================================================
...
def process_account_posts_for_month(instagram_id, account_name, year, month, update_existing=False):
    ...
    PRESTANDAFÖRBÄTTRING v2.0:
    Med modern datumfiltrering är denna funktion nu dramatiskt snabbare.
    Förväntat att gå från minuter till sekunder per månadskörning.
    ...
    try:
        # MODERNISERAD: Hämta posts med server-side datumfiltrering
...
def process_all_accounts_for_month(account_list, year, month, update_existing=False):
    ...
    PRESTANDAFÖRBÄTTRING v2.0:
    Med modern datumfiltrering kommer denna funktion att vara dramatiskt snabbare.
    Förvänta dig sekunder istället för minuter per månadskörning.
    ...
    """
    logger.info(f"Bearbetar alla konton för {year}-{month:02d} (modern datumfiltrering)...")
    ...
    for i, (instagram_id, account_name, facebook_page) in enumerate(account_list):
        ...
        try:
            ...
            # Kort paus mellan konton (mindre nödvändigt med snabbare körningar)
            if i < len(account_list) - 1:
                logger.info(f"Pausar kort innan nästa konto...")
                time.sleep(1)  # Minskat från 2 sekunder
...
def main():
    ...
    MODERNISERAD v2.0:
    Dramatiskt förbättrad prestanda tack vare server-side datumfiltrering.
    Förvänta dig sekunder istället för minuter per månadskörning.
    ...
    parser = argparse.ArgumentParser(
        description="Generera Instagram post-nivå analytics för alla konton och månader (v2.0)",
    ...
    logger.info("Instagram Post Analytics Generator — v2.0 (Moderniserad)")
    logger.info(f"Startdatum: {start_year_month}")
    logger.info("NYTT: Modern datumfiltrering för dramatisk prestandaförbättring")
    logger.info("FÖRVÄNTAT: Sekunder istället för minuter per månadskörning")
    logger.info("-------------------------------------------------------------------")
    ...
    if args.month:
        try:
            ...
            logger.info(f"Kör endast för specifik månad: {year}-{month:02d} (modern datumfiltrering)")
    ...
    logger.info("Modern datumfiltrering kommer att göra detta dramatiskt snabbare än tidigare")
    ...
    for i, (year, month) in enumerate(missing_months):
        ...
        try:
            ...
            # Minimala pauser mellan månader (med modern datumfiltrering behövs knappt pauser)
            if i < len(missing_months) - 1:
                if rate_limit_backoff > 1.5:
                    ...
                    logger.info("Fortsätter direkt till nästa månad (snabb modern API)...")
    ...
    # Visa slutstatistik med prestandajämförelse
    elapsed_time = time.time() - start_time
    avg_rate = api_call_count / (elapsed_time / 3600) if elapsed_time > 0 else 0
    
    logger.info("-------------------------------------------------------------------")
    logger.info("SLUTRESULTAT (v2.0 - MODERNISERAD):")
    ...
    logger.info(f"  - Genomsnitt per månad: {elapsed_time / len(missing_months):.1f} sekunder" if missing_months else "")
    ...
    logger.info("PRESTANDAFÖRBÄTTRING: Sekunder istället för minuter tack vare modern datumfiltrering!")
    logger.info("Klar med Instagram post-analytics v2.0!")
...
# ===================================================================================
# 
# SLUTFÖRT INSTAGRAM POSTS ANALYTICS SCRIPT v2.0 - MODERNISERAD
# 
# REVOLUTIONÄRA FÖRBÄTTRINGAR I v2.0:
# - Använder Instagram Graph API:s inbyggda datumfiltrering (since/until)
# - Eliminerar ineffektiv paginering genom hela kontots historik
# - Dramatisk prestandaförbättring: sekunder istället för minuter
# - Löser alla problem med saknade posts och API-ordning
# - Behåller full bakåtkompatibilitet med befintlig config.py
# 
# Användning:
# python fetch_instagram_posts.py                     # Kör alla saknade månader (SNABBT!)
# python fetch_instagram_posts.py --month 2025-08     # Kör endast augusti 2025 (MYCKET SNABBT!)
...
# - Server-side datumfiltrering via since/until parametrar
# - Ökat limit från 25 till 100 posts per API-anrop
# - Minskat säkerhetsbroms från 500 till 50 sidor
# - Optimerade pauser mellan konton och månader
# - Detaljerad prestandarapportering
# - Kompatibilitet med Graph API v11.0+
# 
# PROBLEMLÖSNINGAR:
# - Eliminerar saknade posts (som "sköldpadda"-problemet)
# - Löser API-ordningsproblem fullständigt
# - Dramatiskt förbättrad prestanda för backlogg-körningar
# - Framtidssäkrad för långvarig användning
# 
# ===================================================================================