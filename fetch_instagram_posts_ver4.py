...
def get_instagram_posts_for_period(instagram_id, since_date, until_date, account_name=None):
    ...
    v2.2 SLUTGILTIG VERSION med alla feedback-förbättringar:
    - Epoch-sekunder för entydiga tidsformat
    - Halvöppet intervall för korrekt gränshantering  
    - Funktionell fallback för server-side filtrering
    - Robust beroende-hantering (pytz/zoneinfo)
    ...
    logger.info(f"Hämtar posts för {display_name} från {since_date} till {until_date} (robust v2.2)")
    
    # ROBUST BEROENDE-HANTERING
    try:
        from datetime import datetime, timedelta
        try:
            import pytz
            ...
            try:
                ...
                logger.info("Använder zoneinfo för tidszonhantering")
            except ImportError:
                logger.critical("Kräver pytz eller Python 3.9+ med zoneinfo. Installera: pip install pytz")
                sys.exit(1)
    except Exception as e:
        logger.error(f"Fel vid import av tidszons-bibliotek: {e}")
        return []
    
    try:
        # KORREKT TIDSZONHANTERING MED HALVÖPPET INTERVALL
        # Startdatum: 00:00 svensk tid
        start_sweden = sweden_tz.localize(datetime.strptime(since_date, "%Y-%m-%d"))
        
        # Slutdatum: HALVÖPPET INTERVALL - första sekunden nästa dag
        end_sweden = sweden_tz.localize(datetime.strptime(until_date, "%Y-%m-%d"))
        next_day_sweden = end_sweden + timedelta(days=1)  # 00:00 nästa dag
        
        # Konvertera till UTC
        start_utc = start_sweden.astimezone(pytz.UTC if 'pytz' in sys.modules else datetime.now().astimezone().tzinfo)
        end_utc = next_day_sweden.astimezone(pytz.UTC if 'pytz' in sys.modules else datetime.now().astimezone().tzinfo)
        
        # EPOCH-SEKUNDER för entydiga API-anrop
        since_epoch = int(start_utc.timestamp())
        until_epoch = int(end_utc.timestamp())
        
        logger.debug(f"  Tidszonkonvertering (v2.2):")
        logger.debug(f"    Sverige: {since_date} 00:00 → {until_date} 24:00 (halvöppet)")  
        logger.debug(f"    UTC epoch: {since_epoch} → {until_epoch}")
        
        # FÖRSÖK SERVER-SIDE FILTRERING FÖRST
        posts = attempt_server_side_filtering(instagram_id, since_epoch, until_epoch, display_name)
        
        # FALLBACK TILL CLIENT-SIDE FILTRERING OM NÖDVÄNDIGT
        if posts is None:
        ...
        # KVALITETSKONTROLL - logga min/max datum för verifiering
        if posts:
...
def attempt_server_side_filtering(instagram_id, since_epoch, until_epoch, display_name):
    ...
    try:
        ...
        while url and page_num < 100:  # Säkerhetsbroms
            ...
            if data and "data" in data:
                ...
                for post in media_in_page:
                    ...
                    if post_timestamp:
                        processed_post = process_post_with_timezone(post, display_name)
                        if processed_post:
        ...
        # DIAGNOSTIK för server-side filtrering
        ...
        if posts_outside_period > posts_in_period and total_posts_found > 50:
            logger.warning(f"    ⚠ Server-side filtrering verkade inte fungera (fler posts utanför än inom)")
...
def fetch_with_client_filter(instagram_id, start_sweden, next_day_sweden, display_name):
    """Fallback med client-side filtrering och robust paginering"""
                ...
                if post_timestamp:
                    try:
                        ...
                        if start_sweden <= post_sweden < next_day_sweden:
                            ...
                            oldest_post_too_old = True
                            
                    except Exception as e:
                        logger.debug(f"      Fel vid tidskonvertering: {e}")
                        continue
            
            hits_in_recent_pages += page_hits
            
            # SMART STOPP-LOGIK: Stoppa om vi gått förbi perioden och har få träffar
            if oldest_post_too_old and hits_in_recent_pages == 0 and page_num > 5:
                logger.debug(f"    Smart stopp efter sida {page_num} (passerat period utan träffar)")
                break
            
            # Återställ träff-räknare var 10:e sida
            if page_num % 10 == 0:
                hits_in_recent_pages = 0
    ...
    logger.info(f"  ✓ Client-side fallback för {display_name}: {len(posts)} posts från {page_num} sidor")
    return posts

def process_post_with_timezone(post, display_name):
    """Bearbeta en post med korrekt tidszonhantering"""
    ...
    try:
        ...
        if media_product_type in ["FEED", "REELS"]:
            ...
            logger.debug(f"      ✗ Filtrerad: {media_product_type}")
            return None
...
def get_existing_post_reports():
    ...
    FIXAD v2.2: Korrekt hantering av filnamnsformat IG_Posts_YYYY_MM.csv
    ...
    for filename in glob.glob("IG_Posts_*.csv"):
        try:
            ...
            # FIXAD: Korrekt antal delar för IG_Posts_YYYY_MM.csv format
